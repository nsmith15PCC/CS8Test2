...
public:
    ...
    vector<tnode<data_type> >& thevector ();


    void insert(const data_type &w, size_t p, size_t l);
    bool remove(const data_type &w);
...
private:
    vector<tnode<data_type> > thetree;
    TRAVERSAL_TYPE thetraversal;

    void insertHELPER (size_t &r, const data_type &w, size_t p, size_t l);
    bool removeHELPER (size_t& r, const data_type &w);
    bool removeMAX (size_t& r, data_type &d, vector<size_t> &p, vector<size_t> &l);
...
{
    thetraversal = t;
    thetree.reserve(20000);
    thetree.push_back(tnode<data_type>());
}
...
template <typename data_type>
void bst<data_type>::insert(const data_type &w, size_t p, size_t l)
{
    insertHELPER(thetree[0].child(1), w, p, l);
}

template <typename data_type>
void bst<data_type>::insertHELPER (size_t &r, const data_type &w, size_t p, size_t l)
{
    if (r)
    {
        if (thetree[r] == w)
        ...
        {
            insertHELPER( thetree[r].child( thetree[r] < w ) , w, p );
        }
    }
    else
    {
        r = thetree.size();
        thetree.push_back(tnode<data_type> (w, p, l));
    }
}
...
template <typename data_type>
bool bst<data_type>::removeMAX (size_t& r, data_type &d, vector<size_t> &p, vector<size_t> &l)
{
    if (thetree[r].child(1))
        return removeMAX(thetree[r].child(1), d, p, l);
    else
    {
        d = thetree[r].data();
...
template <typename data_type>
bool bst<data_type>::removeHELPER (size_t& r, const data_type &w)
{
    if (r)
    {
        if (thetree[r] == w)
            ...
            {
                return removeMAX(thetree[r].child(0), thetree[r].data(), thetree[r].paragraphs, thetree[r].lines);
            }
        else
...
template <typename data_type>
bool bst<data_type>::remove(const data_type &w)
{
